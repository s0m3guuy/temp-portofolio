...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(mysqli_stmt_execute($stmt)) {
        ...
        header("Location: anggota.php?success=1&message=" . urlencode("Data angsuran libur berhasil disimpan!"));
        exit();
    } else {
        $error = mysqli_error($koneksi);
        mysqli_stmt_close($stmt);
        mysqli_close($koneksi);
        header("Location: anggota.php?error=1&message=" . urlencode("Gagal menyimpan: " . $error));
        exit();
    }
