...
document.querySelectorAll('.share-btn').forEach(btn => {
    btn.addEventListener('click', async function() {
        ...
        const canvas = await html2canvas(modalBody, { backgroundColor: '#ffffff', scale: 2 });

        if (window.Android && window.Android.shareImage) {
            // Running inside the wrapped Android app
            const dataUrl = canvas.toDataURL('image/png');
            window.Android.shareImage(dataUrl);
        } else if (navigator.canShare) {
            // Running in a regular mobile browser
            canvas.toBlob(async (blob) => {
                const file = new File([blob], 'bukti-pembayaran.png', { type: 'image/png' });
                if (navigator.canShare({ files: [file] })) {
                    await navigator.share({ files: [file], title: 'Bukti Pembayaran' }).catch(err => console.log(err));
                } else {
